...
class RegisterCourseViewModelTest {
    ...
    @Test
    fun displayCourseData_courseId1() =
        runTest {
            val courseId = 1
    ...
    @Test
    fun updateCourse_courseSuccessfullyUpdated() =
        runTest {
            val courseId = 1
            val collectJob = launch(UnconfinedTestDispatcher()) { subject.uiState.collect() }
            subject.displayCourseData(courseId.toString())
    ...
    @Test
    fun updateCourse_courseNameError() =
        runTest {
            val courseId = 1
            val collectJob = launch(UnconfinedTestDispatcher()) { subject.uiState.collect() }
            subject.displayCourseData(courseId.toString())
    ...
    @Test
    fun setCourseName_courseNameTest() =
        runTest {
    ...
    @Test
    fun setNameProfessor_nameProfessorTest() =
        runTest {
            val nameProfessor = "nameProfessorTest"
            val collectJob = launch(UnconfinedTestDispatcher()) { subject.uiState.collect() }
            subject.setNameProfessor(nameProfessor)
            assertEquals(
                RegisterCourseUiState().copy(nameProfessor = nameProfessor),
                subject.uiState.value,
            )
            collectJob.cancel()
        }
