...
interface PendingOperationDao {
    ...
    @Query("SELECT * FROM pending_operations WHERE status = 'PENDING' AND operation_type = :operationType AND entity_type = :entityType AND user_id = :userId")
    fun getPendingReadOperations(operationType: String, entityType: String, userId: String): Flow<List<PendingOperationEntity>>

    @Query("SELECT * FROM pending_operations WHERE status = :status AND user_id = :userId ORDER BY timestamp ASC LIMIT 1")
