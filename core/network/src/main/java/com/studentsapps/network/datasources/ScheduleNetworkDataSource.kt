...
class ScheduleNetworkDataSource @Inject constructor() {
    ...
    suspend fun getScheduleById(userId: String, scheduleId: String): NetworkSchedule? {
        return try {
            ...
            if (documentSnapshot.exists()) {
                ...
                val startTime = startTimeMap?.let {
                    LocalTime.of(
                        it["hour"]?.toInt() ?: 0,
                        it["minute"]?.toInt() ?: 0,
                        it["second"]?.toInt() ?: 0,
                ...
                val dayOfWeek = documentSnapshot.getString("dayOfWeek").let {
